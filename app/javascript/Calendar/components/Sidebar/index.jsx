  ...
  switch (menu) {
    case "favSessions":
      return (
        <React.Fragment>
          <button
            className="flex font-sans text-sm hover:text-blue-600 cursor-pointer"
    ...
    case "progressions":
      return (
        <React.Fragment>
          ...
          {props.progressions &&
            props.progressions.map((progression) => (
              <ul className="font-sans text-base" key={progression.name}>
                {progression.name}
              </ul>
            ))}
          <ul
    ...
    default:
      return (
        <React.Fragment>
          ...
          <ul
            className="font-sans text-base transition duration-100 ease-in-out transform hover:scale-105 hover:text-blue-600 cursor-pointer dark:text-gray-200"
            onClick={() => setMenu("favSessions")}
          >
            Favourite Sessions
          </ul>
          <ul
            className="font-sans text-base transition duration-100 ease-in-out transform hover:scale-105 hover:text-blue-600 cursor-pointer dark:text-gray-200"
            onClick={() => setMenu("favBlocks")}
          >
            Favourite Blocks
          </ul>
          <ul className="font-sans text-base transition duration-100 ease-in-out transform hover:scale-105 hover:text-blue-600 cursor-pointer dark:text-gray-200">
            Sets and Reps
          </ul>
          <ul className="font-sans text-base transition duration-100 ease-in-out transform hover:scale-105 hover:text-blue-600 cursor-pointer dark:text-gray-200">
            Excercise Libaray
          ...
          <FullPageModal
            ...
            body={
              <React.Fragment>
                ...
                <DropSearch
                  className="bg-white flex items-center border rounded-xl shadow-md w-full py-4 px-6 text-gray-700 leading-tight focus:outline-none"
                  datalist={props.progressions.map(
            ...
            submitFunction={() =>
              createProgression(
                excercise,
                progression,
                DAYS_OF_THE_WEEK.indexOf(ancorDay)
              )
            }
          />
        </React.Fragment>
