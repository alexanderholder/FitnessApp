...
export function saveNewProgression(name, progressions) {
  return async function saveNewProgressionThunk(dispatch, getState) {
    const request = progressions.map((progression) => ({
      sets_reps: progression,
    }));
    const progression_response = await Request.post("/session_progressions", {
      session_progression: { name: name, progressions: request },
    });

    dispatch({
      type: "progressions/progressionsAdded",
      payload: {
        progression: progression_response.data.session_progression,
        sets_reps: progression_response.data.progressions,
      },
    });
  };
}
...
export function createProgressionTemplate(excercise, progression, day) {
  return async function createProgressionTemplate(dispatch, getState) {
    ...
    response.data.workouts.map((workout) =>
      dispatch({ type: "workouts/workoutAdded", payload: workout })
    );
